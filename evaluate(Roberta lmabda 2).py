...
Loads the trained CAP model from Hugging Face Hub and computes classification,
explainability, and bias metrics.
"""
...
class Config:
    SEED = 42
    MODEL_NAME = 'roberta-base'          # backbone used for training
    MAX_LENGTH = 128
    BATCH_SIZE = 16
    NUM_LABELS = 3
    ...
    # Default Hugging Face model repo (replace with your actual repo)
    HF_MODEL_ID = "anonymous-CAP/CAP_Roberta_lambda2"   # ← your HF repo ID
...
def main():
    ...
    parser.add_argument('--model_name', type=str, default=config.MODEL_NAME,
                        help="Backbone for tokenizer (must match the model's base_model_name)")
    parser.add_argument('--model_path', type=str, default=config.HF_MODEL_ID,
                        help="Hugging Face model repo ID (e.g. 'anonymous-CAP/cap-hatespeech')")
    ...
    # ----- CHANGED: Load model from Hugging Face Hub -----
    print(f"Loading model from Hugging Face Hub: {args.model_path}")
    model = AutoModel.from_pretrained(
        args.model_path,
        trust_remote_code=True                # essential for custom modeling_cap.py
    )
    model.to(device)
    model.eval()
